...
public:
    ...
    struct BlendMode
    {
        ...
        explicit BlendMode(bool enabled = false,
            ...
            : BlendEnabled(enabled), LogicOpEnabled(logicOpEnabled), SrcFactor(srcFactor), DstFactor(dstFactor), BlendOp(blendOp), SrcAlphaFactor(srcAlphaFactor), DstAlphaFactor(dstAlphaFactor), AlphaOp(alphaOp), LogicOp(logicOp), WriteRed(writeRed), WriteGreen(writeGreen), WriteBlue(writeBlue), WriteAlpha(writeAlpha)
        {
        }
     ...
     */
    virtual void SetConstBlendFactor(const Vector* constantBlendFactor) = 0;
    virtual const Vector* GetConstBlendFactor() const = 0;
