...
void Plane::Set(const Vector &vN, const Vector &vP)
{
	m_fD      = - ( vN * vP);
	m_vcN     = vN;
	m_vcPoint = vP;
...
void Plane::Set(const Vector &vN, const Vector &vP, float d)
{
	m_vcN = vN;
	m_vcPoint = vP;
	m_fD = d;
...
float Plane::Distance(const Vector &m_vcPoint)
{
	return ( _fabs((m_vcN * m_vcPoint) - m_fD) );
}

int Plane::Classify(const Vector &m_vcPoint)
{
...
int Plane::Classify(const RTMath::Polygon &polygon)
{
