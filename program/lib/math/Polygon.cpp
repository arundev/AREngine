...
void Polygon::Set( const Vector *pPoints, int nNumP,
				   const unsigned int *pIndis, int nNumI)
 {
	 ...
	 m_pPoints = new Vector[nNumP];
	 m_pIndis  = new unsigned int[nNumI];

	 m_NumP = nNumP;
	 m_NumI = nNumI;

	 memcpy(m_pPoints, pPoints, sizeof(Vector)*nNumP);
...
void Polygon::CalcBoundingBox(void) {
	...
	m_Aabb.m_vcMax    = vcMax;
	m_Aabb.m_vcMin    = vcMin;
...
void Polygon::Clip(const Plane &plane, Polygon *pFront, 
					  Polygon *pBack)
{
	if (!pFront && !pBack) return;

	Vector vcHit, vcA, vcB;
	Ray    Ray;
	...
	Vector *pvcFront = new Vector[m_NumP*3];
	Vector *pvcBack  = new Vector[m_NumP*3];
